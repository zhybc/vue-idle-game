    ...
    <div class="map">
      ...
      <div class="dungeons-Info" v-if="dungeons&&!inDungeons">
        ...
        <div class="handle">
          ...
          <div class="dungeons-btn" style="margin-left: 0px;" @click="CircularChallenge()">自动挑战</div>
        </div>
      </div>
      <!-- 副本入口 -->
...
export default {
  ...
  methods: {
    ...
    CircularChallenge(){
      ...
      console.log("自动挑战稍后开始....");
      let timer = setInterval(() => {
        if (!this.inDungeons) {
          const item = this.dungeonsArr[i];
          this.$store.commit("set_sys_info", {
            msg: "自动挑战开始，正在进行第 "+(i+1)+ " 关，共 " + this.dungeonsArr.length + " 关",
            type: 'win'
          });
          console.log("自动挑战开始，正在进行第 "+(i+1)+ " 关，共 " + this.dungeonsArr.length) + " 关";
        ...
        if (i >= this.dungeonsArr.length) {
          // 数组遍历完了,停止定时器
          sum = sum+i
          this.$store.commit("set_sys_info", {
            msg: "该章节副本挑战结束，合计 "+sum+" 关...",
            type: 'win'
          });
          console.log("该章节副本挑战结束，合计 "+sum+" 关...");
          // clearInterval(timer)
          i = 0
          this.createdDungeons()
          this.$store.commit("set_sys_info", {
